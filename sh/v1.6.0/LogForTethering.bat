...
echo adb command can not be found,please re unzip the tools!
pause
exit
)
if exist %PING_CMD% ( echo  - ping ok ) else (
echo ping command can not be found,please re unzip the tools!
pause
exit
)
if exist %JAVA_CMD% ( echo  - java ok ) else (
echo java command can not be found,please re unzip the tools!
pause
exit
)
if exist %FIND_CMD% ( echo  - find ok ) else (
echo find command can not be found,please re unzip the tools!
pause
exit
)
...
echo  - snapshot...
if {%btype%}=={user} (
java -jar %cd%\tools\Screenshot.jar %LOGDIR%\snapshot.png 1>NUL 2>NUL
) else (
...
echo pull NVM count%%i
%ADB_CMD% pull /data/NVM%%i %LOGDIR%\gps
)
) else (
echo  - bt log
mkdir %LOGDIR%\bt
