...
case $d_id in
	'')
		echo "Still is old adb"
		;;
	*)
		echo "alias adb to adb -s"
		adb="adb -s ${d_id}"
		;;
esac
echo ${adb} devices
...
echo  "- check iwconfig execute ok or N"
if [ ! -f $top_dir/wifi_reg/allregs.txt ];then
adb shell trw get legacy 2
adb shell trw get legacy 8
adb pull /data/allregs.txt $top_dir/wifi_reg/
fi

echo  "- NMEA about log"
adb pull  /data/cg/online $top_dir/cg
...
do
    adb pull "$file" $top_dir/ffos/
done

echo  "- get bt logs for-android  4.1_3.4"
mkdir -p "%LOGDIR%/bt"
adb pull /sdcard/btsnoop_hci.log $top_dir/bt/
